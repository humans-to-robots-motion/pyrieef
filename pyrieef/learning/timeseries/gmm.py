...
class GMM(Model):
    ...
    def get_matching_mvn(self, max=False, mass=None):
        if max:
            priors = (self.priors == np.max(
                self.priors)).astype(np.float32)
    ...
    def moment_matching(self, h):
        """
        Perform moment matching to approximate a mixture of
        Gaussian as a Gaussian
    ...
    def concatenate_gaussian(self, q, get_mvn=True, reg=None):
        ...
        if reg is None:
            ...
            if not get_mvn:
                ...
                mvn._lmbda = block_diag(
                    *[np.linalg.inv(self.sigma[i] + reg) for i in q])

                return mvn

    def compute_resp(self,
                     demo=None, dep=None,
                     table=None, marginal=None, norm=True):
                ...
                if dep is None:
                    ...
                    for d in dep:
                        ...
                        B[[i], :] *= multi_variate_normal(
                            demo, mu[i, d], sigma[dGrid][:, :, 0], log=False)
        B *= self.priors[:, None]
        if norm:
    ...
    def init_params_scikit(self, data, cov_type='full'):
        ...
        gmm_init = GaussianMixture(
            self.nb_states, cov_type, n_init=5, init_params='random')
        gmm_init.fit(data)

        self.mu = gmm_init.means_
        if cov_type == 'diag':
            self.sigma = np.array([np.diag(gmm_init.covariances_[i])
                                   for i in range(self.nb_states)])
        else:
    ...
    def init_params_kmeans(self, data):
        ...
        self.sigma = np.array([np.eye(self.nb_dim)
                               for i in range(self.nb_states)])
