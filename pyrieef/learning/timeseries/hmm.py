...
class HMM(GMM):
    ...
    def init_priors(self):
        if self._init_priors is None:
            print colored(
                "HMM init priors not defined, initializing to uniform",
                'red', 'on_white')
    ...
    def trans(self):
        if self._trans is None:
            print colored(
                "HMM transition matrix not defined, initializing to uniform",
                'red', 'on_white')
            self._trans = np.ones(
    ...
    def viterbi(self, demo):
        ...
        for t in range(1, nb_data):
            for i in range(self.nb_states):
                ...
                PSI[i, t] = np.argmax(
                    logDELTA[:, t - 1] + np.log(
                        self.Trans[:, i] + realmin))
                logDELTA[i, t] = np.max(
                    logDELTA[:, t - 1] + np.log(
                        self.Trans[:, i] + realmin)) + logB[i, t]
    ...
    def obs_likelihood(self,
                       demo=None, dep=None, marginal=None,
                       sample_size=200, demo_idx=None):
    ...
    def compute_messages(self,
                         demo=None, dep=None, table=None,
                         marginal=None, sample_size=200, demo_idx=None):
        """

        :param demo:    [np.array([nb_timestep, nb_dim])]
        :param dep:     [A x [B x [int]]] A list of list of dimensions
                Each list of dimensions indicates a dependence of
                variables in the covariance matrix
                E.g. [[0],[1],[2]] indicates a diagonal covariance matrix
                E.g. [[0, 1], [2]] indicates a full covariance matrix
                between [0, 1] and no
        ...
        gamma = (alpha * beta) / np.tile(
            np.sum(alpha * beta, axis=0) + realmin,
            (self.nb_states, 1))

        # Smooth edge marginals. zeta (fast version, considers the scaling
        # factor)
    ...
    def em(self,
           demos, dep=None, reg=1e-8, table=None, end_cov=False,
           cov_type='full', dep_mask=None,
           reg_finish=None):
        """
        ...
        :param demos:   [list of np.array([nb_timestep, nb_dim])]
                        or [lisf of dict({})]
        :param dep:     [A x [B x [int]]] A list of list of dimensions
                Each list of dimensions indicates a dependence of variables
                in the covariance matrix
                E.g. [[0],[1],[2]] indicates a diagonal covariance matrix
                E.g. [[0, 1], [2]] indicates a full covariance matrix
                between [0, 1] and no covariance with dim [2]
        :param reg:     [float] or list [nb_dim x float] for different
        regularization in different dimensions
                Regularization term used in M-step for covariance matrices
        :param table:   np.array([nb_states, nb_demos]) - composed of 0 and 1
                A mask that avoid some demos to be assigned to some states
        :param end_cov: [bool]
                If True, compute covariance matrix without regularization
                after convergence
        ...
        for it in range(nb_max_steps):
            ...
            for n, demo in enumerate(demos):
                s[n]['alpha'], s[n]['beta'], s[n]['gamma'], s[n]['zeta'], s[
                    n]['c'] = HMM.compute_messages(self, demo, dep, table)
    ...
    def condition(self,
                  data_in, dim_in, dim_out,
                  h=None, gmm=False, return_gmm=False):
        if gmm:
            return super(HMM, self).condition(
                data_in, dim_in, dim_out, return_gmm=return_gmm)
        else:
