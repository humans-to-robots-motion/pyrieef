...
class GMR():
    ...
    def predict_GMM(self,
                    sample, input, output, variance_type='v',
                    predict=False, norm=False,
                    reg=1e-9):
        ...
        for i in range(self.gmm.n_components):
            prob[i] = self.get_pdf(
                i, sample,
                has_changed=has_changed, reg=reg) * self.gmm.weights_[i]
            prob_un[i] = self.get_pdf_un(
                i, sample, has_changed=has_changed) * self.gmm.weights_[i]
        ...
        if predict:
            ...
            SigmaOut = np.zeros(
                (len(self.output), len(self.output)), dtype=float)

        MusOut = np.zeros(
            (self.gmm.n_components, len(self.output)), dtype=float)
        SigmasOut = np.zeros(
            (self.gmm.n_components, len(self.output), len(self.output)),
            dtype=float)
    ...
    def predict_histogramm(self, sample, input, output, variance_type='full'):
        ...
        for i in range(self.gmm.n_components):
            prob[i] = self.get_pdf(
                i, sample, has_changed=has_changed) * self.gmm.weights_[
                i]
        ...
        for i in range(self.gmm.n_components):
            ...
            if has_changed:
                self.InvSigmaInIn[i] = linalg.inv(
                    Sigma[np.ix_(self.input, self.input)])
                self.InvSigmaOutIn[i] = np.dot(
                    Sigma[np.ix_(self.output, self.input)],
                    self.InvSigmaInIn[i])
			...
			"""
            if sample.shape != ():
                MuOutTmp = Mu[self.output] + np.dot(
                    self.InvSigmaOutIn[i],
                    (sample[self.input] - Mu[self.input]).T)

                if variance_type == 'full':
                    self.SigmaOutTmp[i] = MuOutTmp ** 2 + (
                        Sigma[np.ix_(self.output,
                                     self.output)] -
                        np.dot(self.InvSigmaOutIn[i],
                               Sigma[np.ix_(self.input,
                                            self.output)])) ** 2
                    ...
                    if has_changed:
                        self.SigmaOutTmp[i] = Sigma[np.ix_(
                            self.output, self.output)] - \
                            np.dot(self.InvSigmaOutIn[i],
                                   Sigma[np.ix_(self.input, self.output)])

            else:
                MuOutTmp = Mu[self.output] + (
                    np.dot(Sigma[np.ix_(self.output, self.input)],
                           self.InvSigmaInIn[i]) * (
    ...
    # @profile
    def predict_local(
            self, sample, input, output, variance_type='full', reg=1e-9):
        """ Gaussian mixture regression with different input for each states

        :param sample: 	np.array((nb_states,)) or
        ...
        for i in range(self.gmm.n_components):
            ...
            if has_changed:
                if sigma_input is None:
                    self.InvSigmaInIn[i] = linalg.inv(
                        Sigma[slii] + 1e-5 * np.eye(len(self.input)))
                else:
                    self.InvSigmaInIn[i] = linalg.inv(
                        Sigma[slii] + sigma_input)
    ...
    def get_pdf(self, i, sample, has_changed=True, reg=1e-9):
        ...
        if has_changed:
            ...
            # 	k += 1
            #
            self.cov_tmp[i] = self.gmm.covars_[i][
                slii] + reg * np.eye(len(self.input))
