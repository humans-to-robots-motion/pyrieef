...
def plot_distpatch(ax, x, mean, var, color=[1, 0, 0], num_std=2, alpha=0.5, linewidth=1,
    ...
    patch = patches.PathPatch(path, facecolor=color,
                              lw=0, edgecolor=color, alpha=alpha)
...
def plot_linear_system(K, b=None, name=None, nb_sub=10, ax0=None, xlim=[-1, 1], ylim=[-1, 1],
    ...
    field = np.einsum('ij,ja->ia', K, mesh_data -
                      b[:, None]) if field is None else field
    ...
    if ax0 is not None:
        ...
        if plot_center:
            if multi_center:
                ctr = ax0.plot(b[:, 0], b[:, 1], 'kx', ms=8, mew=2)[0]
            else:
                ctr = ax0.plot(b[0], b[1], 'kx', ms=8, mew=2)[0]
        ...
        if plot_center:
            if multi_center:
                ctr = plt.plot(b[:, 0], b[:, 1], 'kx', ms=8, mew=2)[0]
            else:
                ctr = plt.plot(b[0], b[1], 'kx', ms=8, mew=2)[0]
...
def plot_mixture_linear_system(model, mode='glob', nb_sub=20, gmm=True, min_alpha=0.,
    ...
    Y, X = np.mgrid[ylim[0]:ylim[1]:complex(nb_sub), xlim[0]:xlim[
        1]:complex(nb_sub)]
    mesh_data = np.vstack([X.ravel(), Y.ravel()])
    statecmap = cmap(range(256))[np.rint(
        np.linspace(0, 255, model.nb_states)).astype(int), :-1]

    if mode == 'glob':
        if gmr:
            field = model.condition(
                mesh_data.T, dim_in=slice(0, 2), dim_out=slice(2, 4))[0]
            model.center = None
        else:
            field = model.condition(
                mesh_data.T) if A is None else model.condition(mesh_data.T, A, b)
        ...
        for i in range(model.nb_states):
            l[:, i] = pbd.multi_variate_normal(
                mesh_data.T, model.mus_in[i], model.sigmas_in[i])
        ...
        # create color map, blending to one color to transparent for each
        # states
        cmaps = [np.ones((256, 4)) for i in range(model.nb_states)]

        # create each cmaps from color of an overall cmap. Each cmaps blends
        ...
        colors = [gamma[:, i].reshape(nb_sub, nb_sub)
                  for i in range(model.nb_states)]
    ...
    if gmm:
        ...
        pbd.plot_gmm(mu, sigma, swap=True, color=statecmap,
                     ax=kwargs.pop('ax0', None), zorder=0)
...
def plot_gmm(Mu, Sigma, dim=None, color=[1, 0, 0], alpha=0.5, linewidth=1, markersize=6,
    ...
    for i, c, a in zip(range(0, nbStates), color, alpha):
        ...
        if ax:
            ...
            if not swap:
                l, = plt.plot(Mu[0, i], Mu[1, i], '.',
                              color=c, alpha=a)  # Mean
            else:
                l, = plt.plot(Mu[i, 0], Mu[i, 1], '.',
                              color=c, alpha=a)  # Mean
                # plt.plot(points[0,:], points[1,:], color=c,
                # linewidth=linewidth , markersize=markersize) # Contour
...
def plot_gaussian(mu, sigma, dim=None, color='r', alpha=0.5, lw=1, markersize=6,
    ...
    points = np.einsum(
        'ij,ja->ia', R, np.array([np.cos(t), np.sin(t)])) + mu[:, None]
...
def plot_trajdist(td, ix=0, iy=1, covScale=1, color=[1, 0, 0], alpha=0.1, linewidth=0.1):
    ...
    for i in range(0, td.n_data):
        ...
        expSigma[i, :, :] = td.covar[st:end, st:end] * \
            covScale + np.eye(td.n_vars) * 1e-5
