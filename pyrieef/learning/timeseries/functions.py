...
def get_dynamical_feature_matrix(n_varspos, n_derivs, n_data, n_samples, dt):
    ...
    a n_varspos*n_data*n_samples vector of data
    points using dt as time discritization.
...
def multi_variate_normal_old(x, mean, covar):
    ...
    covar: [n_vars x n_vars] matrix representing the covariance
        of the distribution
    ...
    if n_vars > 1:
        ...
        scale = np.sqrt(
            np.power((2 * np.pi), n_vars) * (
                abs(np.linalg.det(covar)) + 1e-200))
...
def mvn_pdf(x, mu, sigma_chol, lmbda, sigma=None, reg=None):
    ...
    return -0.5 * np.einsum(
        '...j,...j', dx, np.einsum('...jk,...j->...k', lmbda_, dx)) \
        - D / 2 * np.log(2 * np.pi) - \
        np.sum(np.log(sigma_chol_.diagonal(axis1=1, axis2=2)), axis=1)
...
def multi_variate_t(x, nu, mu, sigma=None, log=True, gmm=False, lmbda=None):
    ...
    if not gmm:
        ...
        dist = np.einsum('...j,...j', dx, np.einsum(
            '...jk,...j->...k', lmbda_, dx))
        # (nb_timestep, )
        ...
        if not log:
            ...
            log_lik = gammaln(
                (nu + p) / 2) + 0.5 * np.linalg.slogdet(lmbda_)[1] - \
...
def multi_variate_normal(x, mu, sigma=None, log=True, gmm=False, lmbda=None):
    ...
    if not gmm:
        ...
        log_lik = -0.5 * np.einsum('...j,...j', dx,
                                   np.einsum('...jk,...j->...k', lmbda_, dx))

        if sigma is not None:
            log_lik -= 0.5 * \
                (x.shape[1] * np.log(2 * np.pi) + np.linalg.slogdet(sigma)[1])
        else:
            log_lik -= 0.5 * \
                (x.shape[1] * np.log(2 * np.pi) - np.linalg.slogdet(lmbda)[1])
